...
static void	save_argv(t_data *data, char **argv)
{
	*(int *) &data->philo_nbr = ft_atol(argv[1]);
	data->fork_nbr = data->philo_nbr;
	*(int *) &data->die_time = ft_atol(argv[2]) * 1000;
	*(int *) &data->eat_time = ft_atol(argv[3]) * 1000;
	*(int *) &data->sleep_time = ft_atol(argv[4]) * 1000;
	if (argv[5])
